        ...
        private void KurKontrol()
        {
            if (frmAnaSayfa.KontrolInternet()==true)
            {
                if (txtDolar.Text == "$" || txtDolar.Text == "")
                {
                    ...
                    XElement dolar = (from s in kurlar.Elements() where s.Attribute("CurrencyCode")?.Value == "USD" select s).Single();
                    XElement euro = (from s in kurlar.Elements() where s.Attribute("CurrencyCode")?.Value == "EUR" select s).Single();
